...
* The following fields are not supported at this time:
...
** *Component*
** *Status* +
The *In Progress* status type may appear twice in the *Status* field. If so, please test both *In Progress* values to find the working value.

* Fields containing the following list components are failing to render:
...
* You must edit or create custom fields prior to creating the JIRA connection in Composer.

* When you create an issue, you may encounter a 400 error in the output metadata. Please ignore this error.
