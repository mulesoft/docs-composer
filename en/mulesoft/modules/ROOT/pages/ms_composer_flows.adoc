...
* When configuring the error handler, you can establish different patterns to meet your requirements. You can: 
** Add all steps in one error handler. +
This pattern is useful if you have the same *On Error* process for any errors or if you can configure multiple *On Error* with error conditions to handle specific types of errors. If any error occurs in the step inside the *Watch* section, the remaining steps in the same *Watch* section will not proceed.
