...
TIP: When using Composer, flows are assigned to specific users. To allow members of your team to share flows, we suggest creating a shared integration user. This allows your team to work simultaneously on flows and to allow multiple people to access the same flows concurrently with provided credentials.
