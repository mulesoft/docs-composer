...
|*Authorization URL* | `+https://auth.atlassian.com/authorize+` | You must register your callback URL when creating a Jira account.  As a result, these these credentials only work in staging environments.
|*Authorization URL Query Parameters*
a|Parameter 1:
...
|Step |Field |Example Value |Notes
.9+|Connect to your HTTP Account |*Connection Display Name* | `+Shopify at dpelaez+shopifydev@mulesoft.com (QA)+` | For more information, see the https://partners.shopify.com/[Shopify Partner Program^].
|*Authorization Type*| `OAuth 2.0 Authorization Code` | The value in this field is fixed.
