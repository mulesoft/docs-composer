...
*** `List`, a value is required. You can enter a value manually. For primitive list types (`String`, `Number`, `Boolean`, `Date`, or `DateTime`), you can create a list statically using the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] or xref:xref:ms_composer_using_actions_in_a_flow.adoc#create-a-picklist-mapping[map in a list data pill]. For List<Objects>, select either: 
...
. In the header, click image:images/setup_icon.png[Setup, 20] > *Setup*. 
