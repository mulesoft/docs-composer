...
Branches are logic-based steps that run vertically in a flow.  You can add multiple steps inside each branch that are executed in sequential order.
...
If you want to set criteria for how the flow executes if none of the existing branches' criteria are met, you can add additional conditions to your If/Else block by clicking *Add condition*.
