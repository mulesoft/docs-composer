...
. In the *Data Type* dropdown, select the data type you want to use for the variable. The following xref:ms_composer_custom_expression_editor.adoc#data-types[data types] are supported: `String`, `Number`, `Boolean`, `Date`, `DateTime` and `List`. If you select List in the Data Type field, in the *List Item Type* field, select:
.. *String*, *Number*, or *Boolean* to return a list containing primitive data types. 
.. *Object* to return a list of objects, for example, the output of the *Get Records* action.
+
The *Label* field appears. Repeat the previous steps until you have added all the required fields. 
...
. Click *Apply*. 
+
The *Create Variables* window displays one or more new variables added to the flow. 
...
After creating a variable, you may select an initial value for it:
...
.. `List`, the default value is defined under each variable. If the field is empty, the value is set to the default value for that data type. You can enter a value manually. For Lists of primitive types (`String`, `Number`, `Boolean`, `Date`, or `DateTime`), you can create a list statically using the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] or <<create-a-picklist-mapping,map in a list data pill>>. For List<Objects>, select either: 
...
.. `List`, you can add items to the start or the end of the list or replace the existing list with a new list. For Lists of primitive types (`String`, `Number`, `Boolean`, `Date`, or `DateTime`), you can create a list statically using the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] or <<create-a-picklist-mapping,map in a list data pill>>. For List<Objects>, select either: 
... *Dynamic List* to create multiple items in the list. Use a list pill to reference the number of items created. Then, map the fields of the elements of the list.
... *Static List*, and then initialize the list with as many items as you need. If you do not add any items, the list is empty by default.
