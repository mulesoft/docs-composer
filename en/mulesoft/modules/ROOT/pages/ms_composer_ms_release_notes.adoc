...
See xref:ms_composer_ms_release_notes_ki.adoc[Known Issues and Limitations for MuleSoft Composer] for a list of known issues and limitations by date.
...
a|To prevent excessive errors from abandoned processes and reduce the risk of service incidents, MuleSoft Composer automatically deactivates flows that experience continuous failures and alerts affected users. Deactivation occurs only when a flow meets both of these conditions:

* 100 or more consecutive errors
* 10 or more days of continuous failure

You can reactivate a deactivated flow at any time.| W-22571502
|===
...
|===
|Issue Resolution |ID
|Jira Connector now correctly updates the watermark for the triggers, and captures all subsequent new and updated issues.| W-20186115
