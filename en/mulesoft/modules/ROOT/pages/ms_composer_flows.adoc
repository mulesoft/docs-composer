...
.. Click *Apply*.
. Click *Save*.
//. Optionally, if the field is enabled for a xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[custom expression], click the pencil to edit the field.
. Click *Save*.
