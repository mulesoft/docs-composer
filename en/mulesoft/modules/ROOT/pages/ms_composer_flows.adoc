...
.. `Format`:  The qualifier for `String` properties that can specify additional data types not natively supported in JSON. For variables, set this value when defining `Date` or `DateTime` variables as `date` or `date-time` respectively. (Optional)
