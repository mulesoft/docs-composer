...
MuleSoft provides a bundle that contains a role that has the necessary permissions required for your Composer integration to NetSuite. To install the bundle, follow these steps:
...
After installing the bundle, generate credentials to configure the connection to Composer. Store the credentials securely since the credentials are visible only once for security purposes. If you need additional information, contact your NetSuite administrator. To generate credentials in the form of tokens, follow these steps:

* Assign the newly created *MuleSoft Composer* role to the desired user.
. Go to *Setup > Users/Roles > Manage Users*.
. Click on the user who will host the integration. You can use a generic account such as `it@company.com` that doesn't necessarily need its own license.
...
* Identify the NetSuite account ID.
. Go to *Setup > Company > Company Information*. You can see the account ID here.
