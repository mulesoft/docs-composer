= Build and a Composer Flow

After you have collected the information as described in xref:ms_composer_checklist.adoc[the preparation checklist], build your flow:

. Create and name a flow.
. In the flow, create a trigger or flow control, and xref:ms_composer_test_flow.adoc[test].
. In the flow, create each action in order. xref:ms_composer_test_flow.adoc[Test] each action before adding the next.
