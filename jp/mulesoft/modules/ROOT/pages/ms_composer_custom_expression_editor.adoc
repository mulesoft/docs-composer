...
コンポーザでデータを操作するには、​_式_​をビルドして​_関数_​を適用します。​*カスタム式エディタ*​を使用して、関数と数学演算子を含む式を作成します。このエディタを使用して、単語を切り捨てたり、語句を大文字にしたり、日付を設定したりします。

フローの数式をビルドするときは、次の点に注意してください。

//More content may be added - to be determined later
* 関数をネストして、関数の別の関数コールの入力パラメータを使用できます。
* コンポーザで使用される Composer 関数は、 https://docs.mulesoft.com/dataweave/2.3/dw-operators[DataWeave]​ を使用してビルドされます。
* Composer 関数は、空白スペースを数式内の空白スペース文字として処理します。

=== 文字列関数

*カスタム式エディタ*​は、次の ​`string`​ 数式と演算子をサポートしています。

[%header%autowidth.spread]
|===

|関数名 |説明 |例 |パラメータ定義 |入力データ型パラメータ |出力データ型
...
|fromStringToDate |文字列入力値を日付値に変換します。形式とロケールを受け入れます。形式パラメータは、入力文字列の形式を表します。 +
...
|Format (形式): 形式パラメータは、入力文字列の形式を表します。このパラメータでは、ISO-8601 に基づいた Java 文字パターンを受け入れます。​`2011-12-03`​ などの日付値の形式は ​`uuuu-MM-dd`​ となります。 +
...
emptyString 関数は、他の関数への入力パラメータとしての使用など、空の文字列が必要な場合に使用できます。 |`-` |`-` | String (文字列)|
...
*カスタム式エディタ*​では、演算子リストから選択された数学演算子 (+、-、*、または /) は、文字列として入力および解決された演算子とは異なります。たとえば、エントリ ​`9+9`​ は ​`18`​ ではなく文字列とみなされます。数学演算子を使用するには、演算子リストから選択するか、​xref:keyboard-shortcuts[キーボードショートカット]​を使用します。

*カスタム式エディタ*​は、次の ​`number`​ 数式と演算子をサポートしています。
...
*カスタム式エディタ*​で、​`date`​ および ​`dateTime`​ データピルは入力としてのみ使用できます。日付を手動で入力することはできません。

*カスタム式エディタ*​は、次の ​`date`​ および ​`dateTime`​ 数式と演算子をサポートしています。
...
|fromDateToString |日付入力値を文字列値に変換します。形式とロケールを受け入れます。形式パラメータは、出力文字列の必須形式を表します。デフォルトの形式は ​`uuuu-MM-dd`​ で、デフォルトのロケールは en-US です。ただし、これらのデフォルトは変更できます。 |`fromDateToString(2021-03-15, uuuu-dd-MM, en-US) = 2021-15-03` +

`fromDateToString(2021-03-15, uuuu/MM/dd, en-US) = 2021/03/15` +

`fromDateToString(2021-03-15, eeee, dd MMMM, ES) = lunes, 15 marzo` |Format (形式): 日付で使用する ISO-8601 形式。出力文字列の目的の形式を表します。たとえば、このパラメータでは Java 8 の java.time.format に基づいた文字パターンを受け入れます。デフォルト形式は ​`uuuu-MM-dd`​ です。 +
...
|fromDateTimeToString |日時入力値を文字列値に変換します。形式とロケールを受け入れます。形式パラメータは、出力文字列の目的の形式を表します。デフォルトの形式は ​`uuuu-MM-dd'T'HH:mm:ssz`​ で、デフォルトのロケールは en-US です。ただし、これらのデフォルトは変更できます。 |`fromDateTimeToString(2021-03-15T23:57:59Z, uuuu-MM-dd'T'HH:mm:ssz, en-US) = 2021-03-15T23:57:59Z` +

`fromDateTimeToString(2021-03-15T23:57:59Z, MMMM dd, ES) = marzo 15` |Format (形式): 日時で使用する ISO-8601 形式。出力文字列の目的の形式を表します。たとえば、このパラメータでは Java 8 の java.time.format に基づいた文字パターンを受け入れます。デフォルト形式は ​`uuuu-MM-dd'T'HH:mm:ssz`​ です。
 +
...
コンポーザ項目が値として受け入れることができる値は、​_データ型_​です。コンポーザでは、次のデータ型を使用できます。
...
|Array (配列) | 同じデータ型の一定数のコンポーネントのコレクションを提供します。
...
|List (リスト) | *[Get Records (レコードの取得)]*​ アクションの出力など、オブジェクトのリストを返します。
...
*カスタム式エディタ*​では、数学演算子のキーボードショートカットを使用できます。数学演算子を挿入するときは、​*Control + Shift + <+、(, )、または *>*​ のいずれかを押します​*。たとえば、演算子を入力するには、*​Ctrl + Shift + * を押します。

*/*​ と ​*-*​ の場合、Shift キーは必要ありません。たとえば、​*/*​ を挿入するには、​*Control + /*​ を押します。

== カスタム式エディタにアクセスする

*カスタム式エディタ*​にアクセスする手順は、次のとおりです。

. *[Composer Flows (コンポーザフロー)]*​ タブから、フローを開きます。
. *[編集]*​ をクリックします。
...
カスタム式を追加するときは、​*[Custom Expression Editor (カスタム式エディタ)]*​ ウィンドウの要素の上にカーソルを置き、役立つ提案やアプリケーション内サポートを含むコンテキストポップオーバーをトリガします。

アクションにカスタム式を追加する手順は、次のとおりです。

. *カスタム式エディタ*​の左ペインで、次のいずれかのタブをクリックします。
...
*カスタム式エディタ*​が表示されます。
. *カスタム式エディタ*​の右ペインにある ​*[<項目名>]*​ ボックスで、既存のカスタム式を選択して削除するか、左ペインから項目を追加する場所にカーソルを置きます。
