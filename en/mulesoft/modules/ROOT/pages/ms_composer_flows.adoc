...
When executing a flow, you can watch for errors in connection or logic in specific steps of your flow and then designate logic to handle the errors that may occur. You can also designate steps to complete additional actions or to send notifications in the event of an error. 
...
You can watch specific steps for errors in connection or logic. If an error occurs, the flow stops normal processing and begins processing the steps in the Error Handler. You can add one or more steps to watch in this section.
...
* When a condition is met in the *On Error* section, the Error Handler processes the conditions in sequential order and stops once the first condition met. After the first condition is met, any subsequent steps are skipped. If you want the process to continue, you can add an additional *On Error* section branch inside your existing *On Error* section.
...
* When an error occurs in a watched action, the error information is saved as a data pill that can then be used for subsequent notification or record-updating actions. For example, if an error occurs, you can designate an action to send an email to your team containing the error information from the data pill.
...
... *Stop*: After processing all steps in the *On Error* section, errors are flagged and the flow stops. The *Status* field of the flow is marked as `FAILED` on the <<flow-details-page,*Run History*>> page.  
... *Continue*: After processing all steps in the *On Error* section, errors are dismissed and the flow continues. If no other errors occur, the *Status* field of the flow is marked as `SUCCESS` on the <<flow-details-page,*Run History*>> page.  
