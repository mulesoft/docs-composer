...
When you use a Salesforce connection in a flow for the first time, if you receive an error that references IP or OAuth failures, that org may have IP restrictions set. Relax the IP restrictions for that connection to resolve this error.
...
. Accept the installation of the connected app in this org by clicking *Install* on the authorization dialog.
+
This does not install the MuleSoft Composer managed package, it merely makes the connected app (the connection you created) available so that you can relax the IP restrictions.
. Click *Edit Policies*.
...
All standard and custom objects are available.
