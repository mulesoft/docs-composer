...
.. *Status*: The status or the flow. The status can be either _SUCCESS_ or _FAILED_.  
.. *Error*: If a flow fails, the error message for the failure appears.  
. The flow actions. You can _delete_, _clone_, or _view_ an active flow.  Note that if the flow is inactive, you can _edit_ the flow.
