...
|Issue |ID
|Fixed the issue in ServiceNow Connector in which some labels were not properly capitalized. |W-10575461
|Fixed the issue in NetSuite Connector in which connections using lowercase values in the `Account ID` field failed. |W-11469671
...
|When deleting a connection, Composer ensures that no flows use that connection. If flows use that connection, they appear in a list in the *Delete Connection* window.  However, in rare cases, flows created or edited before July 12, 2022 may not appear in the list. | |
|===
