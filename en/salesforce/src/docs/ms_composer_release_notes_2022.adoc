...
// ** If a parent field is selected or deselected and it has required child fields, those child fields are also selected or deselected by default.
// ** If a field is selected or deselected, and it has required parent fields, those parent fields are also selected or deselected by default.
...
//* In Slack Connector, the following new action and triggers are available to enable file message attachments:
...
// *The HTTP Connector Scheduler now includes Timestamps.
 ...
 ** When an optional field is selected, the corresponding required fields (including one or more parent fields) are also selected.
 +
Note that required fields can only be removed by deselecting them within the modal. 
