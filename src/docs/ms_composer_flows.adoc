...
When creating actions, null values in fields are handled by:

Null values in custom expressions:


Fields without attributes:
Ignore any field the value for which evaluates to ‘null’, i.e. do not send that field to the connector (exclude from mule XML) (write DW expression such that fields to exclude are omitted from output).
User experience:
For required fields, this would translate into an error message that surfaces to the user.
For optional fields, the field would just get ignored and the user would not be aware of that. This would need to be included in the documentation.

Fields with attributes:
Ignore any attribute the value for which evaluates to ‘null’, i.e. do not send that field to the connector (exclude from mule XML) (write DW expression such that attrubutes to exclude are omitted from output).
User experience:
The attribute would just get ignored and the user would not be aware of that. This would need to be included in the documentation.
