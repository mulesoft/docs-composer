...
After creating the trigger, add one or more actions to the flow. To apply logic to an action, prepend it with an xref:ms_composer-flows.adoc#create-an-if-else-block[If/Else block] or xref:ms_composer-flows.adoc#create-and-test-a-for-each-loop[For Each] loop.
