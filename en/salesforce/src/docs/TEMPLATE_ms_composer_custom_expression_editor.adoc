...
Note that formulas that are duplicated for `Date` and `DateTime` (e.g. `addDays`, `beginningOfMonth`, etc) and are appended with `_Date` or `_DateTime`.
...
. Click *Edit*
. In an action, in a field, select *Custom Expression fx* from the dropdown. +
