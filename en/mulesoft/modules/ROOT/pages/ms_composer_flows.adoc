...
. In the *Variable* section, click *Add Variable*. 
. In the *Visual Editor*, in the *Label* field, enter a unique name for the variable. Note that the name entered is only unique to this step. Values in the *Label* field can be duplicated across different Variable steps. 
...
... `Format`:  The qualifier for `String` properties that can specify additional data types not natively supported in JSON. For variables, this must be set when defining `Date` or `DateTime` variables as `date` or `date-time` respectively. A value in this field is optional. 

//this should be a result statement... but how?! The Dev insisted that we have this section but I have no idea how to format it and I really don't even know how to introduce it.
...
----
//Does anyone have any idea how to indent this block and make the step numbers continue instead of restart?! 
...
. Select the check box next to one or more variables or click *Select all <Number> variables* to select all variables in the list.
. Click *Apply*. +
