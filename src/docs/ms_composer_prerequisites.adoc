...
When using Composer, flows are private to the users who created them. To allow members of your team to collaborate on flows, create a shared integration user. This allows your team to work simultaneously on flows and to allow multiple people to access the same flows concurrently with provided credentials.
