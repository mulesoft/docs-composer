...
You can connect a single-tenant AnyPoint Platform organization to Composer for API Sharing. Using API Sharing, you can expand the connectivity to more systems, have more control over API management and governance, and consume complex APIs built in Anypoint Platform.

To enable API sharing for a single-tenant organization: 

. <<api-sharing>>[Ensure that the shared API appears in the *Shared Apps* section of the canvas] and that you are a <<admin-manage-users>>[Composer Super User].
. In the Composer *Home* page, click *Settings*.
+
The *Account* page appears.
...
+
A Client App named _Composer_ is created within the org. If a Client App named _Composer_ already exists, manually select the Client App to which you want to grant access from the drop-down menu. 
