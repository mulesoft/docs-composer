...
When you use Composer, you build a process that selects information (records, fields, spreadsheet rows, etc) from a source
...
Simply open Composer in your Salesforce org and create a flow, which contains all the steps you need to get data from point A to point B:
...
Or perhaps the flow wakes up every night does its work, looking for new or changed records to copy.
...
Once activated, the flow automatically fires up and does its work when the trigger is fired.
