...
The following tables provide configuration examples and details about required fields when connecting an OAuth Client to a target system:
...
|Connect to your HTTP Account |*Connection Display Name* | | |
| |*Authorization Type*| | |
| |*Base URL* | | |
| |*Client ID* | | |
| |*Client Secret* | | |
...
|Test HTTP Connection |*Request Method* | | |
| |*Request Path* | | |
