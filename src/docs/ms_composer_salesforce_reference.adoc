...
Whether you are using Salesforce as a data source (take new Salesforce records and copy them to a Google Sheet, for example) or a data target (take a Google Sheet row and turn it into a Salesforce lead), some details of how to work with data in a flow are specific to the data source or data target. The details required also depend on the trigger or actions you choose.
