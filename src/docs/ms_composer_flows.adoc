...
When adding decision steps, the *Field* value list is populated based on values selected when defining the trigger for your flow. After the *Field* value is selected, all compatible options appear in the <<supported-operators,*Operator*>> list. If the selected *Operator* requires a second data pill to complete the condition, then the *Value* field is enabled and populated based on the values selected when defining the trigger.
