...
When using a connector with OAuth validation, That’s why we have the ‘refresh’ flow in place, when a OAuth dance is performed a refresh token is granted in order to be able to get new (and fresh) access tokens.
