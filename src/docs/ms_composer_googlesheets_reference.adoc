...
NOTE: Once text is present in one cell or column, updating those cells or columns does not trigger as a new row. To trigger a new row when updating a cell or column, use the *Update row in sheet* trigger.
