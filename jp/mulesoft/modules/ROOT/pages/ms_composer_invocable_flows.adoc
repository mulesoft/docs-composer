...
. <<create-an-invocable-composer-flow, 呼び出し可能コンポーザーフローを作成します>>​。 
. <<invoke-the-composer-flow-from-an-external-system,外部システムからコンポーザーフローを呼び出し>>​ます。
...
. <<configure-your-invocable-composer-flow,呼び出し可能コンポーザーフローを設定します>>​。
. <<test-your-invocable-composer-flow,呼び出し可能コンポーザーフローをテストします>>​。
. <<activate-your-invocable-composer-flow,呼び出し可能コンポーザーフローをアクティブ化します>>​。
...
. コンポーザーにアクセスします。
. xref:ms_composer_flows.adoc#create-a-new-flow[新しいフローを最初から作成]​します。
...
*** `[String (文字列)]`​、​`[Number (数値)]`​、​`[Boolean (ブール)]`​、​`[Date (日付)]`​、​`[DateTime (日時)]`​ の場合、値は必須です。手動で値を入力したり、フローから既存データピル (変数など) を選択したり、​<<create-a-picklist-mapping,選択リストマッピング>>​ (​`文字列`​変数の場合) を選択したり、​xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*カスタム式エディター*]​を選択してカスタム式を作成したりできます。 
*** `[List (リスト)]`​ の場合は、値は必須です。値を手動で入力できます。プリミティブリスト型 (​`[String (文字列)]`​、​`[Number (数値)]`​、​`[Boolean (ブール)]`​、​`[Date (日付)]`​、または ​`[DateTime (日時)]`​) の場合、​xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*カスタム式エディター*]​または​<<create-a-picklist-mapping,リストデータピルのマップ>>​を使用して静的にリストを作成できます。List<Objects> の場合、次のいずれかを選択します。 
...
. フローを完了します。 
+
たとえば、​xref:ms_composer_flows.adoc[「コンポーザーフローの構築」]​で説明されているように、フローのシステムアクションを設定したり、変数を定義したり、フローコントロールを作成したりします。 
. *[Save (保存)]*​ をクリックします。
...
. <<set-up-salesforce-for-invocable-flows,呼び出し可能フロー用に Salesforce をセットアップします>>​。
. <<set-up-flow-builder-for-invocable-flows,呼び出し可能フロー用に Flow Builder をセットアップします>>​。
...
. <<set-up-salesforce-for-invocable-flows-orch,呼び出し可能フロー用に Salesforce をセットアップします>>​。
. <<set-up-flow-orchestration-for-invocable-flows,呼び出し可能フロー用にフローオーケストレーションをセットアップします>>​。
...
. <<set-up-salesforce-for-invocable-flows-apex,呼び出し可能フロー用に Salesforce をセットアップします>>​。
. <<use-apex-classes-to-trigger-invocable-flows,Apex クラスを使用して呼び出し可能フローをトリガーします>>​。
