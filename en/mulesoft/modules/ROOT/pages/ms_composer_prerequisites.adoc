...
* Your Anypoint organization doesn’t employ https://docs.mulesoft.com/api-manager/2.x/configure-multiple-credential-providers#how-using-multiple-external-client-providers-works[external client providers].
...
. Optionally, on the *Connected Apps* page, if you would like to remove Composer’s access to a single API, click the options button next to the API and then click *Remove*.
