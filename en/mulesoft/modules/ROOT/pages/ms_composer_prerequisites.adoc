...
. Log in to Anypoint Platform. Ensure that the user logging into Anypoint Platform has the right https://docs.mulesoft.com/api-manager/latest/access-client-app-id-task[permissions to create a client app].
...
.. *Composer Super User*: This user can create and manage their own flows. Additionally, this user can take ownership and manage any flows in the Anypoint Composer org. The xref:general::automation-credits-usage-and-rates.adoc[Usage Summary] is  only available to those with the Composer Super User role.
