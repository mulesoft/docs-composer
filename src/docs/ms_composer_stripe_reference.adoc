...
* Ensures that all API requests must be made via HTTPS.
...
Create Customer::
Create a new customer. When creating a new customer, note that the `Phone` field is the customer's phone number and not the shipping phone number.
