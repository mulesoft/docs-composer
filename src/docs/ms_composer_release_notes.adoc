...
* After you create a trigger, you can't create a new connection for the trigger.
+
To change to a new connection on a trigger that's already been created, use one of the following workarounds:
//CAPP=-1550
+
** If you want to change the data source, for example switching from Salesforce to Workday, delete the trigger.
...
* An error occurs if you use the Salesforce action *Get Records* and return a Date field.
//CPLAT-1126

* It's possible to create a flow that starts an infinite loop. Exercise caution whenever your flows use the same connection for a trigger and an action to ensure you don't create an infinite loop.
//CPLAT-785
