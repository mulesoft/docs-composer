...
Sage Intacct Connector をコンポーザを統合するには、最初に Sage Intacct で次の操作を完了する必要があります。

* https://developer.intacct.com/web-services/[Web サービスのサブスクリプション]​を Sage Intacct の ​*[Subscription (サブスクリプション)]*​ セクションで有効化する。​*Web サービスのサブスクリプション*​は、コンポーザが Sage Intacct API と連携するために必要です。
* https://developer.intacct.com/web-services/[会社の​*送信者 ID*​ を Sage Intacct の ​*[Web Services Authorizations (Web サービス認証)]*]​ セクションに追加する。Sage Intacct は、​*送信者 ID*​ でコンポーザを識別します。使用する Sage Intacct インスタンスで Mulesoft ​*送信者 ID*​ を認証する必要があります。
* Sage Intacct への接続に https://developer.intacct.com/api/company-console/roles/[必要な権限を作成して割り当てる]​。アカウントのセキュリティを保証するため、権限はコンポーザからアクセスする必要があるモジュールに制限してください。Sage Intacct で次の操作を行います。
...
Sage Intacct にログインするために使用するユーザ ID。
...
添付ファイルレコードのリストを返します。リストを絞り込むには、​*[Attachment ID (添付ファイル ID)]*​、​*[Attachment Name (添付ファイル名)]*​、または ​*[Folder Name (フォルダ名)]*​ の値を指定します。リスト全体を返すには、これらの項目を空白のままにしておきます。

Create Attachment Folder (添付ファイルフォルダの作成)::

添付ファイルの新しいフォルダを作成します。​*[Name (名前)]*​ の値を指定します。

Get Attachment Folders (添付ファイルフォルダの取得)::

添付ファイルフォルダのリストを返します。リストを絞り込むには、​*[Folder Name (フォルダ名)]*​ の値を指定します。リスト全体を返すには、これらの項目を空白のままにしておきます。
...
コンポーザフローの自動化でカスタム項目が必要な場合は、 https://developer.intacct.com/customization-services/#custom-fields[Sage Intacct のドキュメント]​を参照してください。
