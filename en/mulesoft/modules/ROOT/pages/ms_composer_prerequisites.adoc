...
* Name
+
Enter a nickname for this IDP.
+
* Client Registration URL
+
This field appears when *Dynamic Registration* is selected. <Need info>.
+
...
* Client ID
+
This field appears when *Manual Registration* is selected. The unique identifier that you provided for your manually created client application.
+
* Client Secret
+
This field appears when *Manual Registration* is selected. The password, or secret, for authenticating your Composer organization with your Identity Provider.
+
...
. Optionally, expand *Advanced Settings*, and provide the folllowing values:
+
* Group Scope
+
<Need info>
+
* Group Attribute JSON Data Expression
+
<Need info>
+
* Disable
...
. Click *Save*.
. Log out of Composer, navigate to the sign-on URL you entered in the Identity Management SAML 2.0 form, and then log in through your identity provider to test the configuration.
