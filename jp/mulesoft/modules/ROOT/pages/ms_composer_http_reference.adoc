...
NOTE: HTTP 応答タイムアウトは 60 秒で、変更できません。
...
ヘッダーが ​*Content-Type: application/json*​ の場合、リクエストボディの一部として有効な JSON ({}、"") が最低限必要です。[] は、リクエストボディのルートではサポートされていません。

|*Query Parameters (クエリパラメーター)* |Object (オブジェクト) |要求に含めるクエリパラメーター。クエリパラメーターを追加する場合、​*[Key (キー)]*​ および ​*[Value (値)]*​ 項目の値を入力する必要があります。

|*Example Request (サンプル要求)* |String (文字列) |JSON 形式のサンプル要求。この項目の値を入力すると、コンポーザーはサンプルを解析して、その項目を推定します。その後、その項目を使用してデータを入力し、要求の操作を設定できます。コンポーザーによってサンプルから推定された項目を表示するには、​*[Select Additional Fields (追加項目を選択)]*​ をクリックします。この項目で指定されたサンプル JSON 値は、​*[Example Response (サンプル応答)]*​ 項目で指定された値とは異なっている​_必要_​があります。

|*Example Response (サンプル応答)* |String (文字列) |JSON 形式のサンプル応答。この項目の値を入力すると、コンポーザーはサンプル JSON ペイロードを解析して、JSON 項目に一致するデータピルを提供します。コンポーザーによってサンプルから推定された項目を表示するには、​*[Select Additional Fields (追加項目を選択)]*​ をクリックします。この項目で指定されたサンプル JSON 値は、​*[Example Request (サンプル要求)]*​ 項目で指定された値とは異なっている​_必要_​があります。 
