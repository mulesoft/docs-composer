...
. In the *Data Type* dropdown, select the data type you want to use for the variable. The following xref:ms_composer_custom_expression_editor.adoc#data-types[data types] are supported: `String`, `Number`, `Boolean`, `Date`, `DateTime` and `List`.
...
.. `Type`: The data type of the variable. The following xref:ms_composer_custom_expression_editor.adoc#data-types[data types] are supported: `String`, `Number`, `Boolean` and `Array`. To define a `Date` or `DateTime` variable, the data type should be set to `String` and the `Format` property should be defined. (Required) 
...
.. `Items`: Specifies the `Item` type for `List` variables. `Items` is an object and accepts the following child properties: (Required only when defining List variables) 
... `Type`: The data type of the variable. The following data types are supported for List items:  `String`, `Number`, `Boolean`, and `Object`. If defining an `Object`, you must also define the properties of that `Object`. (Required)
... `Properties`: The child properties of an object and is required if the `List Item Type` is `Object`. In the `Properties` object, each property key is the name of the property in the `Object List Item` and is set to an object that defines its data type. For more information in defining the data type of object child properties, refer to `Type` (step 4b) and `Format` (step 4c). (Optional)
...
After creating a variable, you must select an initial value for it:

. Create a new variable using either the <<create-a-variable-using-the-visual-editor,Visual Editor>> or the <<create-a-variable-using-the-json-schema,JSON Schema>>.
...
.. `List`, in the the *List Item Type* field:
... Select *String*, *Number*, or *Boolean* to return a list containing primitive data types. 
... Select *Object* to return a list of objects, for example, the output of the *Get Records* action. 
+
The *Label* field appears. Repeat the previous steps until you have added all the fields required to initialize your variable. 
... Click *Apply* and then select either: 
...
.... *Static List*, and then initialize the list with as many items as you need. If you do not add any items, the list be empty by default.
...
.. `String`, `Number`, `Boolean`, `Date`, or `DateTime`, you can enter a value manually, select any existing data pill (including variables) from your flow, select a <<create-a-picklist-mapping,Picklist Mapping>> (for `String` variables) or you can select the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] to create a custom expression. Note that if you are updating a variable with a value created from another variable, do not update them in the same step. Variables are not necessarily updated in the same order in which they are listed. To avoid unexpected behavior, update each variable in separate steps.
.. `List`, you can add items to the start or the end of the list or replace the existing list with a new list. You can either add those items statically using static list mapping or dynamically by mapping to another list.
