...
. In the *Data Type* dropdown, select the data type you want to use for the variable. The following xref:ms_composer_custom_expression_editor.adoc#data-types[data types] are supported: `String`, `Number`, `Boolean`, `Date`, `DateTime`.
...
. Specify the initial value for each variable. The default value is defined under each variable. If the field is empty, the value is set to the default value for that data type. You can manually enter a value, select any existing data pill (including variables) from your flow, or select the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] to create a custom expression. 
+
...
+
Note that: 
.. Each variable is defined within a `Properties`  object with a `Variable Unique ID`.  The `Variable Unique ID` enables the variable to be tracked within your flow. The `Variable Unique ID` can contain only alphanumeric characters, hyphens, and underscores. When using the Visual Editor, this ID is automatically generated for you.
...
=======
... `Type`: The data type of the variable. The following xref:ms_composer_custom_expression_editor.adoc#data-types[data types] are supported: `String`, `Number`, and `Boolean`. To define a `Date` or `DateTime` variable, the data type should be set to `String` and the `Format` property should be defined. A value is required. 
... `Format`:  The qualifier for `String` properties that can specify additional data types not natively supported in JSON. For variables, this value must be set when defining `Date` or `DateTime` variables as `date` or `date-time` respectively. (Optional)

>>>>>>> e31c2332b79f42fe88a2ad701154cfbd3c5f7699
...
. In an action card, in a field that supports *Pick from List*, select *Picklist Mapping* from the menu. +
...
. Optionally, to add additional field mappings, click *Add Row* and enter the source field values manually (with a row representing each value) and select the target mappings from the dropdown.
