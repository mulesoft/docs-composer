...
As an administrator, you can enable users to add identity providers for their organization. Note that:
...
* *Client Registration URL*: This field appears when *Dynamic Registration* is selected. The URL to dynamically register client applications as a client application for your identity provider.
* *Authentication Header*: This field appears when *Dynamic Registration* is selected. The header to provide credentials to authenticate with a server. This header is required if the provider restricts registration requests to authorized clients.
...
* *Group Attribute JSON Data Expression*: The JSONata Expression used to select the groups from the user info or ID token. The result must be an array of strings.
* *Disable server certificate validation* checkbox: Select checkbox to disable server certificate validation if your OpenID client management instance presents a self-signed certificate, or one signed by an internal certificate authority.
...
+
* *Name*: Enter a nickname for this IDP.
* *Sign On URL*: Redirect URL provided by the IDP for sign in, for example: `https://example.com/sso/saml`.
