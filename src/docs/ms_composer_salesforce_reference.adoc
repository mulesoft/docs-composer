...
[NOTE]
For the purposes of comparing flows, if you initiate a date comparison when building a flow and if there is no value in the date fields, then the value in the date fields will be updated to `0001-01-01` by default.
...
When you add a record that was returned from the *Get records* action, you must add a xref:ms-composer-flows.adoc#create-and-test-a-for-each-loop[For Each loop] to your flow.
