...
. The *Run History* of the flow. Composer stores up to 1000 records per flow and can include only the last two weeks of data. For example, for a flow that produces 10 records per day, you can see the Run History for the last two weeks of data (14 x 10 = 140 records). For a flow that produces 100 records per hour, you can see no more than 10 hours of the Run History.
...
You can define variables in your Composer flow. Defining a variable allows you to complete certain use cases like aggregating calculations and storing values. Variables are defined locally within a flow and can be used anywhere within that flow. 
...
The *<Flow Name>* page appears.
. Click *Edit*. 
. Directly above the position in your flow to which you want access the Variable step, click the plus button. 
...
. In the *Action* dropdown, click *Create Variable*.
. In the *Variable* section, select *Add Variable*. 
. In the *Visual Editor*, in the *Label* field, enter a unique name for the variable. Note that the name is only unique to the step. Values in the *Label* field can be duplicated across different Variable steps. 
. In the *Data Type* drop-down, select the data type you want to use for the variable. The following xref:ms_composer_custom_expression_editor.adoc#data-types[data types] are supported: `String`, `Number`, `Boolean`, `Date`, `DateTime`.
...
. Optionally, to make changes to the new variables, click *Manage Variables* to open the Visual Editor or *Edit Schema* to open the JSON Schema. 
. Complete the initial value for each variable. The default value is defined under each variable. If you leave the field empty, the value is set to the default value for that data type. You can enter a value manually, select any existing data pill (including variables) from your flow, or you can select the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] to create a custom expression. 
...
. <<access-the-variable-step,Access the Variable step.>> 
. In the *Action* dropdown, click *Create Variable*.
. In the *Variable* section, select *Create Schema*. 
. In the *JSON Schema* window, enter the properties for the variable you want to create. Note that: 
.. Each variable is defined within a `Properties`  object with a `Variable Unique ID`.  The `Variable Unique ID` allows the variable to be tracked within your flow. The `Variable Unique ID` can only contain alphanumeric characters, hyphens, and underscores. When using the Visual Editor, this ID is automatically generated for you.
Each variable must specify the `Name` and data type. To set the properties for the variable definition, complete the following:
... `Variable Label (Title)`:  The display label for the variable. A value is required. 
... `Type` : The data type of the variable. The following xref:ms_composer_custom_expression_editor.adoc#data-types[data types] are supported: `String`, `Number`, and `Boolean`. To define a `Date` or `DateTime` variable, the data type should be set to `String` and the `Format` property should be defined. A value is required. 
... `Format`:  The qualifier for `String` properties that can specify additional data types not natively supported in JSON. For variables, this must be set when defining `Date` or `DateTime` variables as `Date` or `DateTime` respectively. A value in this field is optional. 
The properties are entered into the JSON Schema window. For example: +
...
----
//Does anyone have any idea how to indent this and make the step numbers continue instead of restart?! 
. Optionally, to add additional variables in the JSON Schema, enter them in the window.
. Click *Apply*. +
The *Create Variables* window appears, displaying one or more new variables added to the flow. 
. Optionally, to make changes to the new variables, click *Manage Variables* to open the Visual Editor or *Edit Schema* to open the JSON Schema.
. Complete the initial value for each variable. The default value is defined under each variable. If you leave the field empty, the value is set to the default value for that data type.  You can enter a value manually, select any existing data pill (including variables) from your flow, or you can select the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] to create a custom expression. 
...
. <<access-the-variable-step,Access the Variable step.>> 
. In the *Action* dropdown, click *Update Variable*.
...
. Select the check box next to one or more variables or click *Select all <Number>* variables to select all variables in the list and then click *Apply*. +
The *Update Variable* step appears. 
. Update the value for each variable. You can enter a value manually, select any existing data pill (including variables) from your flow, or you can select the xref:ms_composer_custom_expression_editor.adoc#add-custom-expression-to-an-action[*Custom Expression Editor*] to create a custom expression. Note that if you are updating a variable with a value created from another variable, do not update them in the same step. Variables are not necessarily updated in the same order in which they are listed. To avoid unexpected behavior, update each variables in separate steps. 
