...
Note that when selecting a record type, the `Netsuite: Retrieving metadata for the step failed` error sometimes occurs. For more information, see the https://issues.salesforce.com/issue/a028c00000zHtne/mulesoft-composer-fails-with-netsuite-retrieving-metadata-for-the-step-failed-after-3-minute-wait[knowledge article].
