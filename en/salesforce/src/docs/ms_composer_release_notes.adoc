...
| Previously, in the Google Sheets Connector when Google Sheets was the trigger, all output fields were  returned as `text` regardless of the assigned  data type (ex., `int`, `bool`, etc.).  As a result, customers attempting to map the trigger data pills to non-text fields in the action encountered an error. Now, trigger fields retain their original data types and can be mapped appropriately in subsequent actions. | SE-227701, COCN-575
| Fixed the issue in the Google Sheets Connector in which metadata from a document containing a space failed.  | COCN-742
| Fixed the issue in the Asana connector in which an error appeared when establishing a connection between Asana and Composer. | SE-12452
