...
NOTE: You must add the *Automated Process* user to the *Permission Sets* assignments. For more information, refer to https://help.salesforce.com/s/articleView?id=sf.perm_sets_mass_assign.htm&type=5[Assign a Permission Set to Multiple Users]. 
...
. If you select a Record-Triggered Orchestration, select the object, configure the trigger, and set entry conditions. 
. Click *Save*.
