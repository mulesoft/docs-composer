...
Salesforce 接続アプリケーションと認証についての詳細は、 https://help.salesforce.com/articleView?id=remoteaccess_request_manage.htm[Salesforce ドキュメント]​を参照してください。
...
+
image:composer::change-connection.png["[Change the Connection (接続を変更)] ボタン", 500]
...
+
image:composer::get-records-salesforce.png["Salesforce Connector の [Get Records (レコードの取得)] サンプルアクション設定", 500]
+
