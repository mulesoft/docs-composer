...
.. *Function* tab to add a function.
.. *Data* tab to add a field or data pill.
.. *Operator* tab to add an operator. +
The available functions, fields, data pills, or operators appear.
...
. Once you have created the expression, select *Check Syntax*.
