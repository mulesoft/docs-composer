...
* `+https://api.hubapi.com/application/settings/test/v1/actions/accounts-fetch+`
 ...
 |For more information, see the https://developers.hubspot.com/docs/api/oauth-quickstart-guide[Hubspot OAuth] documentation.
...
. In the What Should Start This Flow? dialog, select HTTP.
...
.. Authorization URL Query Parameters: The key-value pairs that appear after the question mark in the URL. Under the OAuth Authorization Code Grant type, the authorization request sends data as query parameters.  Composer handles these query parameters automatically, but sometimes additional ones are convenient or necessary. For example, the Google API needs access_token=offline to emit a refresh token. Without this parameter, a connection will expire after an hour.
.. Token Exchange URL: The URL where the authorization code can be exchanged for an access token.
.. Token Refresh URL (Optional): The URL where the token can be refreshed.  This field is only applicable if the application marks the tokens as expired after a certain period of time.
