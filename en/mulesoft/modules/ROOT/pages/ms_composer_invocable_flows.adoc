...
. Add an assignment. Click *+* to add a new element and then click *Assignment*.
. Enter a *Label* and *API Name*.
. Set variable values. For example, to set a value from a Salesforce record to an opportunity ID variable, the *Variable* is `{!OpportunityId.opportunityId}`, the *Operator* is `equals`, and the *Value* is `{!$Record.id}`.
